...
impl<'a> Parser<'a> {
    ...
    fn try_parse_divider(&mut self) -> Option<Span> {
        ...
        if let Some(end_pos) = after_first.find("===") {
            ...
            let end_eq_count = after_first[end_pos..]
                .chars()
                .take_while(|&c| c == '=')
                .count();
    ...
    fn parse_tag_parts(&self, content: &str) -> (String, Vec<String>, Vec<String>) {
        ...
        if let Some(space_pos) = content.find(' ') {
            ...
            let complex_attrs: Vec<String> = rest
                .split(' ')
        ...
        if let Some(eq_pos) = content.find('=') {
            ...
            let attrs: Vec<String> = attrs_str.split(',').map(|s| s.trim().to_owned()).collect();
...
mod tests {
    ...
    fn test_plain_text() {
        ...
        assert!(
            matches!(&content.spans[0].kind, SpanKind::Plain { text } if text == "Hello world")
        );
    }
    ...
    fn test_tag_with_attribute() {
        ...
        assert_eq!(content.spans.len(), 1);
        if let SpanKind::Tagged {
            tag, attributes, ..
        } = &content.spans[0].kind
        {
            assert_eq!(tag, "url");
